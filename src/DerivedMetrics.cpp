...
template<typename T>
std::unique_ptr<Metric> DerivedMetrics::calculateRate(std::string name, T type)
{
        ...
        if (search != cache.end())
        {
                ...
                if (size >= 2)
                {
                        ...
                        return std::unique_ptr<Metric>(new Metric(rate, name + "Rate", cache[name].back()->getEntity(), cache[name].back()->getTimestamp()));
                }
        }
        return nullptr;
}
...
template <>
std::unique_ptr<Metric> DerivedMetrics::calculateRate(std::string name, std::string type)
{
        return nullptr;
}

template<typename T>
std::unique_ptr<Metric> DerivedMetrics::calculateAverage(std::string name, T type)
{
        T total = 0;
        ...
        return std::unique_ptr<Metric>(new Metric(average, name + "Average", cache[name].back()->getEntity(), cache[name].back()->getTimestamp()));
}

template <>
std::unique_ptr<Metric> DerivedMetrics::calculateAverage(std::string name, std::string type)
{
        return nullptr;
}

template<typename T>
std::unique_ptr<Metric> DerivedMetrics::processMetric(T value, std::string name, std::string entity, std::chrono::time_point<std::chrono::system_clock> timestamp)
{
        auto search = cache.find(name);
