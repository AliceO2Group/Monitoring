...
BOOST_AUTO_TEST_CASE(derivedAverage)
{
	...
	derivedHandler.registerMetric(DerivedMetricMode::AVERAGE, name);
		
	for (auto const result : results)
	{
		std::chrono::time_point<std::chrono::system_clock> timestamp = std::chrono::system_clock::now();
		std::unique_ptr<Metric> derived = derivedHandler.processMetric(result.value, name, entity, timestamp);
...
BOOST_AUTO_TEST_CASE(derivedRate)
{
        ...
        DerivedMetrics derivedHandler;
        derivedHandler.registerMetric(DerivedMetricMode::RATE, name);
